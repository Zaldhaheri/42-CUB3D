...
int	init_textures(char *file, t_data *data, unsigned int texture[64][64])
{
	void	*image;
	char	*address;
	int		info[3];
	...
	if (ft_strncmp(file + ft_strlen(file) - 4, ".xpm", 5))
		return (write(1, "Error: Texture extension should be '.xpm'.\n", 44),
			0);
	image = NULL;
	image = mlx_xpm_file_to_image(data->mlx, file, info + 1, info + 2);
	if (!image)
		return (0);
...
int	setting_textures(t_data *data, char **part)
{
	char	*path;
	...
	if (!ft_strncmp(part[0], "NO", 3))
		return (init_textures(path, data, data->parsing->so));
	else if (!ft_strncmp(part[0], "SO", 3))
		return (init_textures(path, data, data->parsing->no));
	else if (!ft_strncmp(part[0], "WE", 3))
		return (init_textures(path, data, data->parsing->we));
	else if (!ft_strncmp(part[0], "EA", 3))
		return (init_textures(path, data, data->parsing->ea));
	return (1);
}

int	color_and_texture(t_data *data, char **content)
{
	int index;
	char **part;
